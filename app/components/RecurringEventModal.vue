...
const props = defineProps<{ event: RecurringEvent; tables: Table[] }>()
const emit = defineEmits<{
  close: []
  updated: [event: RecurringEvent]
  deleted: [id: string]
}>()

const { canEditBookings } = usePermissions()

const item = ref<RecurringEvent>({ ...props.event })
...
const save = async () => {
  ...
  try {
    const res = await $fetch<{ recurringEvent: RecurringEvent }>(`/api/recurring-events/${item.value.id}`, {
      method: 'PATCH',
      body: editDraft.value
    })
    item.value = res.recurringEvent
    emit('updated', res.recurringEvent)
  } catch (err: any) {
...
const deleteEvent = async () => {
  ...
  try {
    await $fetch(`/api/recurring-events/${item.value.id}`, { method: 'DELETE' })
  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4" @click.self="emit('close')">
    <div class="max-h-[90vh] w-full max-w-sm overflow-y-auto rounded-2xl bg-lyktan-paper p-6 shadow-xl">
      ...
      <template v-if="canEditBookings">
        <div class="space-y-4">
          ...
          <label class="block">
            ...
            <select v-model.number="editDraft.weekday" class="w-full rounded-lg border border-black/15 px-3 py-2 text-sm">
      ...
      <dl v-else class="space-y-3 text-sm">
        <div>
          <dt class="text-[0.72rem] font-medium text-lyktan-mute">Veckodag & tid</dt>
          <dd class="text-lyktan-ink">{{ WEEKDAY_LABELS[item.weekday] }} {{ item.start_time.slice(0, 5) }}–{{ item.end_time.slice(0, 5) }}</dd>
        </div>
        <div>
